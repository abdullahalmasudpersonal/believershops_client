...
const AttarComboOffers = () => {
    ...
    const nestedProductsLength = nestedProducts.filter(categore => categore.subCategory === 'Attar Combo Offer').length;
    

    return (
        <div className='nestedProductsMain'>
             <PageTitle pageTitle='Attar Combo Offer |' />
            <div className='nestedProductsBreadcrumb'>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 ">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Attar Combo Offers</li>
                    </ol>
                </nav>
            </div>
            <div className='nestedProductsNamePart'>
                <div>
                    <h5 className='m-0 fw-bold'>Attar Combo Offer Products In Mahsez</h5>
                </div>
                <div>
            ...
            <div className='nestedProductsSortByViewPart'>
                <div>
                    <p className='m-0'>{nestedProductsLength} Products Found In Attar Combo Offer</p>
                </div>
            ...
            <div className='nestedProducts'>
                {
                    nestedProducts.filter(categore => categore.subCategory === 'Attar Combo Offer').slice(0).reverse().map(nestedProduct => <NestedProduct key={nestedProduct._id} nestedProduct={nestedProduct}></NestedProduct>)
                }
            </div>
        </div>
    );
