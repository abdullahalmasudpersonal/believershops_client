...
const AlifAttars = () => {
    ...
    const nestedProductsLength = nestedProducts.filter(categore => categore.subCategory === 'Alif Attar').length;

    return (
        <div className='nestedProductsMain'>
             <PageTitle pageTitle='Alif Attar |' />
            <div className='nestedProductsBreadcrumb'>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 ">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Alif Attar</li>
                    </ol>
                </nav>
            </div>
            <div className='nestedProductsNamePart'>
                <div>
                    <h5 className='m-0 fw-bold'>Alif Attar Products In Mahsez</h5>
                </div>
                <div>
            ...
            <div className='nestedProductsSortByViewPart'>
                <div>
                    <p className='m-0'>{nestedProductsLength} Products Found In Alif Attar</p>
                </div>
            ...
            <div className='nestedProducts'>
                {
                    nestedProducts.filter(categore => categore.subCategory === 'Alif Attar').slice(0).reverse().map(nestedProduct => <NestedProduct key={nestedProduct._id} nestedProduct={nestedProduct}></NestedProduct>)
                }
            </div>
        </div>
    );
