...
const Tazbeehs = () => {
    ...
    const nestedProductsLength = nestedProducts.filter(categore => categore.category === 'Tasbeeh').length;
    

    return (
        <div className='nestedProductsMain'>
             <PageTitle pageTitle='Tasbeeh |' />
            <div className='nestedProductsBreadcrumb'>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 ">
                        ...
                        <li class="breadcrumb-item active">Tasbeeh</li>
                    </ol>
                </nav>
            </div>
            <div className='nestedProductsNamePart'>
                <div>
                    <h5 className='m-0 fw-bold'>Tasbeeh Products In Mahsez</h5>
                </div>
                <div>
            ...
            <div className='nestedProductsSortByViewPart'>
                <div>
                    <p className='m-0'>{nestedProductsLength} Products Found In Tasbeeh</p>
                </div>
            ...
            <div className='nestedProducts'>
                {
                    nestedProducts.filter(categore => categore.category === 'Tasbeeh').slice(0).reverse().map(nestedProduct => <NestedProduct key={nestedProduct._id} nestedProduct={nestedProduct}></NestedProduct>)
                }
            </div>
        </div>
    );
