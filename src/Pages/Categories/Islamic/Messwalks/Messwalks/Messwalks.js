...
const Messwalks = () => {
    ...
    const nestedProductsLength = nestedProducts.filter(categore => categore.category === 'Miswak').length;
    

    return (
        <div className='nestedProductsMain'>
             <PageTitle pageTitle='Miswak |' />
            <div className='nestedProductsBreadcrumb'>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 ">
                        ...
                        <li class="breadcrumb-item"><Link to='/categore/islamic/miswaks' className='breadcrumbItem'>Islamic</Link></li>
                        <li class="breadcrumb-item active">Miswak</li>
                    </ol>
                </nav>
            </div>
            <div className='nestedProductsNamePart'>
                <div>
                    <h5 className='m-0 fw-bold'>Miswak Products In Mahsez</h5>
                </div>
                <div>
            ...
            <div className='nestedProductsSortByViewPart'>
                <div>
                    <p className='m-0'>{nestedProductsLength} Products Found In Miswak</p>
                </div>
            ...
            <div className='nestedProducts'>
                {
                    nestedProducts.filter(categore => categore.category === 'Miswak').slice(0).reverse().map(nestedProduct => <NestedProduct key={nestedProduct._id} nestedProduct={nestedProduct}></NestedProduct>)
                }
            </div>
        </div>
    );
